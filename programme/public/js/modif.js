...
function veriftype() {
  ...
  if(parseInt(inter)>1){
    // 
    // alert(inter);
    for (let i = 1; i < parseInt(inter); i++) {
      var t = 'Interrogation '+i;
      typeSelect.innerHTML += '<option value="'+i+' . '+t+'"\>'+t+'</option\>';
    }
  ...
  if(parseInt(dev)>4){
    // 
    for (let i = 4; i < parseInt(dev); i++) {
      var t = 'Devoir '+(i - 3);
      typeSelect.innerHTML += '<option value="'+i+' . '+t+'"\>'+t+'</option\>';
    }

  }
  if(parseInt(inter)==1 && parseInt(dev)==4){
    // 
    ...
    imp.innerHTML = 'Aucune Note disponible ';
  }
}
